...
for i = 1:N
    ...
    % Adjusted text placement for Sylhet to prevent overlapping with top title
    if i == 9 % Sylhet
        text(x_locs(i)+5, y_locs(i)-18, sprintf('%s\n(%.1f hr)', depots{i}, travel_time_hr(i)), ...
...
ylim([-120, 200]); % Set upper limit margin for top labels
...
try
    ...
    saveas(fig, fullfile(results_dir, 'logistics_framework.png'));
    fprintf('\n[SUCCESS] Updated visualization saved to Results directory.\n');
catch
    try
        saveas(fig, 'logistics_framework.png');
        fprintf('\n[INFO] Saved updated visualization in working directory.\n');
    catch
        fprintf('\n[INFO] Figure generated successfully.\n');
    end
end